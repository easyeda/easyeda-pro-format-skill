...
| expandWidth | `number \| null` | ✓ | - | 展开宽度：maskExpand 为 true 时区域的宽；可空，null 表示未设置（缺省为 null） |
| expandHeight | `number \| null` | ✓ | - | 展开高度：maskExpand 为 true 时区域的高；可空，null 表示未设置（缺省为 null） |
...
→ [查看示例](../../examples/SIMULATION/t-sch-mask-region.md)
