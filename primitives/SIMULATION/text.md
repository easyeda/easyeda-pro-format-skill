...
**原理图族里的一段画布文字**，一行一条（`type:"TEXT"`）：图上写的说明 / 注释，
只按自己的坐标与字体样式绘制，**不挂宿主**（这一点与器件属性 [TSchAttr](./attr.md) 不同）。

样式来自 `TFontStyle`；文本锚在 `x` / `y` 上，`rotation` 是**绕该点逆时针**的角度。
`version` 为 `'2.0'` 表示这条数据由 2.0 转换而来——两版的**文本原点解析不一致**，靠它区分。

⚠️ 本类型也是 CIRCLE / RECT / ELLIPSE 内嵌 `text` 字段的来源，但**盘上内嵌的那份是缩减形态**：
`align` 与 `groupId` / `locked` / `zIndex` 恒为 `null`，另多一个本类型**没有收录**的
`lineHeight`（行高倍数，`null` = 主题默认）——那是有意保留的扩展字段，解码端会读。
...
| x | `number` | ✓ | - | 文本坐标 X（0.01 inch） |
| y | `number` | ✓ | - | 文本坐标 Y（0.01 inch） |
| rotation | `number` | ✓ | min: 0, max: 360 | 旋转角度（角度制，**逆时针为正**），绕 文本坐标 旋转 注意：本字段在 RECT / ELLIPSE 上是**图形旋转角的实际载体**——这两个类型的 顶层 `rotation` 已废弃（写盘恒 0），旋转值写在这里；CIRCLE 没有顶层 rotation 字段，旋转角同样由这里承载。 |
...
→ [查看示例](../../examples/SIMULATION/t-sch-text.md)
