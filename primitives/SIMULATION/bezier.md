...
**原理图族里的一条三阶贝塞尔曲线**，一行一条（`type:"BEZIER"`）：曲线形状由 `controls` 决定——
扁平坐标数组、每 2 个数一组 `x y`，一条曲线 4 个点（起点、控制点 1、控制点 2、终点）。

该数组可容纳**多段**曲线：段数为 N 时共 `3N + 1` 个点，第 3k 个点既是上一段的终点、
同时是下一段的起点。⚠️ 元素可以是 `null`（写入端按段穿插空位），读侧按可空处理，别当坏数据。

曲线是**开口**的（不像 [TSchPoly](./poly.md) 有 `closed`）；样式来自 `TLineStyle`。
...
| controls | `(number \| null)[]` | ✓ | - | 控制点：扁平坐标数组，每 2 个数为一组 `x y`；三阶贝塞尔一条曲线 4 个点， 依次为 起点、控制点 1、控制点 2、终点。 该数组可容纳**多段**曲线：段数为 N 时共 `3N + 1` 个点；第 3k 个点（k ≥ 1） 是第 k 段的终点，同时兼作第 k+1 段的起点。 ⚠️ **数组元素可以是 `null`**：写入端按段穿插空位（`JSON.stringify` 后即为 `null`）， 读取侧需按可空处理，不要把 `null` 当坏数据。 |
...
→ [查看 JSON Schema](../../schemas/t-sch-bezier.json)
...
→ [查看示例](../../examples/SIMULATION/t-sch-bezier.md)
