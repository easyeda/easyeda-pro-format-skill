...
**原理图族里的折线 / 多边形**，一行一条（`type:"POLY"`）：由 `points` 顺序相连成一张图形，
`closed` 为真时把**结束点自动连回起始点**（闭合之后填充才围得住区域）。

`startShape` / `endShape`（3.3 新增）给两端加箭头 / 燕尾等端头，解码时字段缺失会被补 `null`。

⚠️ **同名不同义**：封装域也有一个 `POLY`，但那边存的是路径串而不是点数组，
字段与取值都不通用，别照另一套写。
...
| startShape | [EBothEndShape](../REFERENCE/e-both-end-shape.md) \| null | ✓ | default: null | 取值范围：NONE（无）、ARROW（箭头）、SOLID_ARROW（实心箭头）、TAIL（尾巴）、SOLID_TAIL（燕尾箭头）、CIRCLE（圆形）、SQUARE（方形）、RHOMBUS（菱形） |
| endShape | [EBothEndShape](../REFERENCE/e-both-end-shape.md) \| null | ✓ | default: null | 取值范围：NONE（无）、ARROW（箭头）、SOLID_ARROW（实心箭头）、TAIL（尾巴）、SOLID_TAIL（燕尾箭头）、CIRCLE（圆形）、SQUARE（方形）、RHOMBUS（菱形） |
...
→ [查看示例](../../examples/SYMBOL/t-sch-poly.md)
