...
**原理图族里的一个椭圆**，一行一条（`type:"ELLIPSE"`）：由中点 `centerX` / `centerY` 与水平 /
垂直半径 `radiusX` / `radiusY` 定义。

⚠️ 与 [TSchRect](./rect.md) 同款坑：**顶层 `rotation` 是遗留字段（写盘恒 0）**，真实旋转角写在
**内嵌 `text.rotation`**；解析判据是 `Number.isFinite(+data.text.rotation)`，而 `+null === 0`
也算有限值——内嵌 `text.rotation` **必须写数字**，写 `null` 会把旋转**静默置 0**。

内嵌 `text` 是缩减形态（`align` 等恒为 `null`，另带 `lineHeight` 扩展字段），见 [TSchText](./text.md)；
`radiusX` / `radiusY` 真机存在负值（2.0 迁移样本），解码端一律取绝对值，生成时写正数即可。
...
| centerX | `number` | ✓ | - | 中点 centerX（0.01 inch） |
| centerY | `number` | ✓ | - | 中点 centerY（0.01 inch） |
| radiusX | `number` | ✓ | min: 0 | 水平半径 radiusX ⚠️ **真机存在负值**（样本 `"radiusX":2.5` 配 `"radiusY":-2.5`，来自 2.0 迁移）。 解码端一律取绝对值，负号无意义；**生成时写正数即可**。（0.01 inch） |
| radiusY | `number` | ✓ | min: 0 | 垂直半径 radiusY ⚠️ **真机存在负值**（样本 `"radiusX":2.5` 配 `"radiusY":-2.5`，来自 2.0 迁移）。 解码端一律取绝对值，负号无意义；**生成时写正数即可**。（0.01 inch） |
...
→ [查看示例](../../examples/SCH_PAGE/t-sch-ellipse.md)
