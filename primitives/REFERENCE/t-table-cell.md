...
注意：这是**内嵌结构**，不是数据流里的独立图元（表格的原子类型 TABLE 由 [TSchTable](../SCH_PAGE/table.md) 描述），
...
| lineHeight | `number \| null` | ✓ | min: 0 | 行高倍数：相对字号的倍数（默认 1.2），实际行高 = lineHeight × fontSize；null 表示用主题默认 （**写入端未设置时即产出 `null`**，判空一律用 `== null`） ⚠️ **它不是长度、没有单位**——别拿「0.01 inch」之类的口径去乘它。 |
...
→ [查看示例](../../examples/REFERENCE/t-table-cell.md)
