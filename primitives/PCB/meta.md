...
PCB 文档（`PCB`）的 META 行，一行一条（`type:"META"`）：文档名、来源、排序，
外加 `board`（**所属板子**）与 `parent`（**父 PCB**，用于分区 / 子 PCB 场景；无父级时写空串）。

PCB 文档的 META 行 id 是**固定单例名** `META`（属**固定单例 id**，见 [TSingletonElementId](../REFERENCE/t-singleton-element-id.md)）。
...
| parent | [TDocUuid](../REFERENCE/t-doc-uuid.md) | ✓ | - | 父 PCB |
| title | `string` | ✓ | - | 文档名称 |
