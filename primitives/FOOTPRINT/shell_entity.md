...
3D 外壳域里的**实体 / 挖槽区域**，一行一条（`type:"SHELL_ENTITY"`）：在外壳上
**增加实体**（如结构加强）或**挖出槽口**（让位、开孔），两者由 `type` 区分。

它是不是落在**侧面**，取决于有没有绑定侧面基准线：`refs` 里引用了 [TPcbCrease](./crease.md)
的就是**侧面挖槽**，没绑定的则按垂直方向作用于**顶面 / 底面**。

⚠️ 它**取代了已废弃的** [TPcbShellCut](./shellcut.md)（外壳挖槽区域），后者只保留解析兼容。
...
| type | [EEntityType](../REFERENCE/e-entity-type.md) | ✓ | - | 取值范围：SLOT（挖槽）、ENTITY（实体） |
| entityBelong | [EEntityBelong](../REFERENCE/e-entity-belong.md) |  | - | 取值范围：AUTO（自动）、UPPER_SHELLS（上壳）、LOWER_SHELLS（下壳） |
| slotOptions | [ESlotOption](../REFERENCE/e-slot-option.md)[] |  | - | 挖槽 - 选项。 内部的位标志 `Outline=4` / `ScrewPillar=8` / `Entity=16` 会被映射成这三个字符串。 |
| depth | `number` | ✓ | - | 深度 |
| strokeWidth | `number` | ✓ | - | 线宽（mil） |
| path | [TPcbComplexPolygon](../REFERENCE/t-pcb-complex-polygon.md) | ✓ | - | 3D 实体轮廓（**复杂多边形**，即 [TPcbComplexPolygon](../REFERENCE/t-pcb-complex-polygon.md)）：外层数组的每个元素是一条子路径， 每个元素本身又是一个单多边形（[TPcbSinglePolygon](../REFERENCE/t-pcb-single-polygon.md)）。 读法为 **【外轮廓 + 内洞】**，且**只取首个子路径** （解码器的 `finish()` 会执行 `path = path[0]`，后续内洞当前不会被使用）。 |
