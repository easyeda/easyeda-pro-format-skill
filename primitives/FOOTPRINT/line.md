...
PCB / 封装域里的**直线段**，一行一条（`type:"LINE"`）：由**两个端点 + 线宽**
确定的一条线段，是画布上最通用的「两点一线」图元。

**走线（导线）**就是它；丝印线、装配线、机械层的辅助线也都用它。
它属于某一层，并可在 `netName` 上标注所属网络。

⚠️ 它只能是一条**直线**，表达不了弧：圆弧要用 [TPcbArc](./arc.md)，
而要保留「连续的一条线」的概念则用 [TPcbPoly](./poly.md)。
...
→ [查看示例](../../examples/FOOTPRINT/t-pcb-line.md)
