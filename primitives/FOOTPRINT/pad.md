...
| num | `string` | ✓ | - | 焊盘编号 |
| centerX | `number` | ✓ | - | 焊盘原点 X（mil） |
| centerY | `number` | ✓ | - | 焊盘原点 Y（mil） |
| padAngle | `number` | ✓ | - | 焊盘旋转角度（角度制，**逆时针为正**） |
...
| padOffsetX | `number` | ✓ | - | 孔偏移 X（mil） |
| padOffsetY | `number` | ✓ | - | 孔偏移 Y（mil） |
| relativeAngle | `number` | ✓ | - | 孔相对焊盘旋转角度（角度制，**逆时针为正**） |
| plated | `boolean` | ✓ | - | plated 是否金属化孔壁 |
| padType | [EPadFuncType](../REFERENCE/e-pad-func-type.md) | ✓ | - | 取值范围：NORMAL（普通焊盘）、TEST（测试点）、MARKER（标识点） |
...
| connectMode | [EPadConnect](../REFERENCE/e-pad-connect.md) \| null | ✓ | - | 取值范围：DIVERGENCE（热焊（发散））、DIRECT（直连）、NON_CONNECT（无连接） |
| spokeSpace | `number \| null` | ✓ | - | 热焊-发散间距：null 为遵循规则，其他数据定义同设计规则 |
| spokeWidth | `number \| null` | ✓ | - | 热焊-发散线宽：null 为遵循规则，其他数据定义同设计规则（mil） |
| spokeAngle | `number \| null` | ✓ | - | 热焊-发散角度（角度制，**逆时针为正**）：null 为遵循规则，其他数据定义同设计规则 |
...
| refs | [TElementId](../REFERENCE/t-element-id.md)[] |  | - | **反向**关联的图元编号：记录的是「哪些图元引用了本焊盘」 （与 TPcbFill/TPcbShell*.refs 的「本图元引用了谁」方向相反） |
