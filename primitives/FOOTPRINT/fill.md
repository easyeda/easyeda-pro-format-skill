...
PCB / 封装域里的**填充区域**，一行一条（`type:"FILL"`）：在一块封闭外形里铺出一片
图形，`fillStyle` 决定它是**实心填充**、**网格填充**还是**内电层填充**。

它还可能是**桥接铜**（`isBridgingCopper` + `networkList`），并用 `refs` 记下自己
引用了哪些图元。

⚠️ `path` 虽是复杂多边形，读法却是**【并列】**：各子路径是**彼此独立**的多边形，
**不是**「外轮廓 + 内洞」的嵌套结构（单次画只有一个，组合模式画才有多个）。
...
| width | `number` | ✓ | - | 线宽（mil） |
| fillStyle | [EPcbFillStyle](../REFERENCE/e-pcb-fill-style.md) | ✓ | - | 取值范围：SOLID（实心填充）、GRID（网格填充）、PLANE（内电层填充） |
| path | [TPcbComplexPolygon](../REFERENCE/t-pcb-complex-polygon.md) | ✓ | - | 填充外形（**复杂多边形**，即 [TPcbComplexPolygon](../REFERENCE/t-pcb-complex-polygon.md)）：外层数组的每个元素是一条子路径， 每个元素是一个单多边形（[TPcbSinglePolygon](../REFERENCE/t-pcb-single-polygon.md)）。 - 对于单次画的多边形，这里只有一个单多边形 - 对于组合模式画的多边形，这里才有多个单多边形 读法为 **【并列】**：各元素是**彼此独立**的多边形， **不是**「外轮廓 + 内洞」的嵌套结构。 |
| isBridgingCopper | `boolean` |  | - | 是否是桥接铜 |
| networkList | `string[]` |  | - | 桥接网络 |
| refs | [TElementId](../REFERENCE/t-element-id.md)[] |  | - | **正向**关联的图元编号：记录的是「本填充引用了哪些图元」（如引用的焊盘 id 放在首位）。 【联动增删·解引用不删对方】被引用的图元被删除时，本图元**保留**， 只把引用字段清空并重新上传（不是连带删除）。 |
...
→ [查看示例](../../examples/FOOTPRINT/t-pcb-fill.md)
